...
import { Projector, ArrowLeft, ArrowRight } from 'lucide-react';
...
const TvPage = () => {    
    ...
    const API_URL = `https://api.themoviedb.org/3/tv/top_rated?${API_KEY}&page=${currentPage}&with_origin_country=IN`;
  ...
  return (
    <div className="container mx-auto px-4">
        <h1 className="text-[8vw] md:text-[5vw]  text-center uppercase font-extrabold mt-32 mb-10 ">Popular TV-Series</h1>    
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-5 gap-5 md:gap-10">
          {trending.map((item, index)=>(
              <MovieSingle item={item} key={index} showMedia={false} />
          ))}         
        </div>
        <hr />
